...
async def test_load_table_to_memory_basic(settings, patch_get_pool):
    ...
    result = await load_table_to_memory("users", settings, limit=10, where_clause="active = $1", params=[True])
...
async def test_load_query_to_memory(settings, patch_get_pool):
    ...
    result = await load_query_to_memory("SELECT COUNT(*) as count FROM orders", settings)
...
@pytest.mark.asyncio
async def test_bulk_insert_dicts_default_chunk(settings, sample_records_dict, patch_get_pool):
    mock_pool, mock_conn = patch_get_pool
...
@pytest.mark.asyncio
async def test_bulk_insert_different_chunk_sizes(settings, sample_records_dict, patch_get_pool):
...
@pytest.mark.asyncio
async def test_bulk_insert_tuples_requires_columns(settings, sample_records_tuples, patch_get_pool):
    mock_pool, mock_conn = patch_get_pool
    ...
    inserted = await bulk_insert(
        "users", sample_records_tuples, settings, columns=["id", "name", "email"], batch_size=1
    )
    assert inserted == 2
...
@pytest.mark.asyncio
async def test_bulk_insert_error_propagates(settings, sample_records_dict, patch_get_pool):
    mock_pool, mock_conn = patch_get_pool
